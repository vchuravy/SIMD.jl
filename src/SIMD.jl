...
vectortype(::Type{T}, N) = LLVM.API.LLVMVectorType(convert(LLVMType, T), N)
...
@generated function vstore(v::Vec{N,T}, ptr::Ptr{T},
    ...
    vtyp = VectorType(typ, N)

    btyp = convert(LLVMType, Bool)
    vbtyp = VectorType(btyp, N)
    ...
    T_i1 = LLVM.Int1Type(JuliaContext())
    T_i32 = LLVM.Int32Type(JuliaContext())
    llvm_f, _ = create_function(T_void, [vtyp, PointerType(typ), vbtyp], "vstore")
    mod = LLVM.parent(llvm_f)
    mStoreID = LLVM.lookup_id("llvm.masked.store.*")
    mStore = LLVM.intrinsic_decl(mod, mStoreID, [vtyp, PointerType(vtyp), T_i32, VectorType(T_i1, N)])
    
    # generate IR
    Builder(JuliaContext()) do builder
        entry = BasicBlock(llvm_f, "entry", JuliaContext())
        position!(builder, entry)
        ptr = bitcast!(builder, parameters(llvm_f)[2], PointerType(vtyp))
        mask = trunc!(builder, parameters(llvm_f)[3], VectorType(T_i1, N))
        call!(builder, mStore, [parameters(llvm_f)[1], ptr, ConstantInt(T_i32, align), mask])
        ret!(builder)
    end
    
    call_function(llvm_f, Void, Tuple{Vec{N, T}, Ptr{T}, Vec{N, Bool}}, :( (v, ptr, mask) ))
end
